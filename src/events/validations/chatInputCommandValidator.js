  ...
  try {
    ...
    if (commandObject.devOnly) {
      if (!developersId.includes(interaction.member.id)) {
        const rEmbed = new EmbedBuilder()
          ...
          .setDescription(`${msgConfig.commandDevOnly}`);
    ...
    if (commandObject.testMode) {
      if (interaction.guild.id !== testServerId) {
        const rEmbed = new EmbedBuilder()
          .setColor(`${msgConfig.embedColorError}`)
          .setDescription(`${msgConfig.commandTestMode}`);
        interaction.reply({ embeds: [rEmbed], ephemeral: true });
        return;
      };
    };
    ...
    if (commandObject.userPermissions?.length) {
      for (const permission of commandObject.userPermissions) {
        ...
        const rEmbed = new EmbedBuilder()
          .setColor(`${msgConfig.embedColorError}`)
          .setDescription(`${msgConfig.userNoPermissions}`);
        interaction.reply({ embeds: [rEmbed], ephemeral: true });
        return;
      };
    };
    ...
    if (commandObject.botPermissions?.length) {
      for (const permission of commandObject.botPermissions) {
        ...
        const rEmbed = new EmbedBuilder()
          .setColor(`${msgConfig.embedColorError}`)
          .setDescription(`${msgConfig.botNoPermissions}`);
        interaction.reply({ embeds: [rEmbed], ephemeral: true });
        return;
      };
    };
